    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('course_name') ? 'is-invalid' : '' }}" type="text" name="course_name" id="course_name" value="{{ old('course_name', $course->course_name) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Course::COURSE_LEVEL_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('course_level') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="course_level_{{ $key }}" name="course_level" value="{{ $key }}" {{ old('course_level', $course->course_level) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('course_modules') ? 'is-invalid' : '' }}" name="course_modules[]" id="course_modules" multiple required>
                    @foreach($course_modules as $id => $course_module)
                        <option value="{{ $id }}" {{ (in_array($id, old('course_modules', [])) || $course->course_modules->contains($id)) ? 'selected' : '' }}>{{ $course_module }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('course_duration') ? 'is-invalid' : '' }}" type="text" name="course_duration" id="course_duration" value="{{ old('course_duration', $course->course_duration) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('course_total_fee') ? 'is-invalid' : '' }}" type="number" name="course_total_fee" id="course_total_fee" value="{{ old('course_total_fee', $course->course_total_fee) }}" step="0.01">
