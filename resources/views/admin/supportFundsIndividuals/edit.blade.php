            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('member_no') ? 'is-invalid' : '' }}" name="member_no_id" id="member_no_id" required>
                    ...
                    @foreach($member_nos as $member_no)
                        <option value="{{ $member_no->id }}" {{ (old('member_no_id') ? old('member_no_id') : $supportFundsIndividual->member_no->id ?? '') == $member_no->id ? 'selected' : '' }}>{{ $member_no->member_no }} | {{ $member_no->member_name->name }}</option>
                    @endforeach
                </select>
