                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('member_name') ? 'is-invalid' : '' }}" name="member_name_id" id="member_name_id" required>
                                    @foreach($member_names as $member_name)
                                        <option value="{{ $member_name->id }}" {{ (old('member_name_id') ? old('member_name_id') : $membershipsIndividual->member_name->id ?? '') == $member_name->id ? 'selected' : '' }}>{{ $member_name->name }} | {{ $member_name->id_no}}</option>
                                    @endforeach
                                </select>
...
<script>
    $(document).ready(function(){
        var id = {!! $membershipsIndividual->application_no->id !!};
        runAjax(id);
    })
