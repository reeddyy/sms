...
class AddRelationshipFieldsToPaymentsQualificationsTable extends Migration
{
    public function up()
    {
        Schema::table('payments_qualifications', function (Blueprint $table) {
            ...
            $table->foreign('enrolment_no_id', 'enrolment_no_fk_4947763')->references('id')->on('enrolments_qualifications');
            $table->unsignedBigInteger('payment_source_id');
            $table->foreign('payment_source_id', 'payment_source_fk_4947764')->references('id')->on('payment_sources');
        });
    }
}
